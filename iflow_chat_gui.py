...
class IflowChatGUI(QMainWindow):
    ...
    def _create_sidebar(self) -> QWidget:
        ...
        bottom_widget.setFixedHeight(130)
        bottom_layout = QVBoxLayout()
        bottom_layout.setContentsMargins(12, 8, 12, 12)
        bottom_layout.setSpacing(8)
        
        # 设置按钮
        settings_btn = ModernButton("⚙️ 设置", primary=False)
...
- 用户说"帮我点击屏幕上的某个按钮" -> 先 @screenshot() 查看屏幕，然后 @request_control() 获取权限，再 @screenshot() 确认位置，最后 @mouse_move() 和 @mouse_click() 执行操作，完成后 @screenshot() 查看结果"""
    ...
    def _load_conversation(self, filename: str):
        ...
        if loaded:
            ...
            QMessageBox.warning(self, "错误", "加载对话失败")
...
"""
    ...
    def _import_extension(self):
        ...
        try:
            ...
            QMessageBox.information(self, "成功", "扩展导入成功！\n请重启程序以加载新扩展。")
    ...
    def _delete_extension(self):
        ...
        if not current_item:
            QMessageBox.warning(self, "警告", "请先选择要删除的扩展")
            return
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                if os.path.exists(ext_dir):
                    ...
                    QMessageBox.information(self, "成功", "扩展删除成功！\n请重启程序以生效。")
                else:
                    QMessageBox.warning(self, "错误", "扩展目录不存在")
